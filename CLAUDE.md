...
| 标书编制 | `/bid` | AI解析招标文件、框架生成、内容编写、协作、废标检查 |
...
- AI: Claude API（招标文件解析、标书内容生成）
- 文件处理: python-docx（Word生成）、PyPDF2（PDF解析）
...
- **后端端口**: :8002（建筑ERP用:8000，棋育用:8001）
- **服务目录**: `/opt/bid-system/`
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| Phase 0 | 项目初始化、脚手架搭建 | 待开始 |
| Phase 1 | 基础模块（认证+权限+组织架构+审批引擎+数据字典） | 待开始 |
| Phase 2 | 招标管理 + 投标决策 + 开标跟踪 | 待开始 |
| Phase 3 | 企业资料库（资质/业绩/人员/产品） | 待开始 |
| Phase 4 | 标书编制工作台（AI解析+框架生成+内容编写+废标检查） | 待开始 |
| Phase 5 | 标书知识库 + 投标复盘 + 数据看板 | 待开始 |
| Phase 6 | 部署上线 | 待开始 |
