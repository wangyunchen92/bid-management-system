...
5. chapters 中的 template 字段：找招标文件中"响应文件格式""投标文件格式""附件""格式一/二/三..."等部分，把对应章节的模板原文原样摘出（保留表格、空白下划线 ___、签章位 (签章) 等）；招标文件没提供模板则填空字符串并把 matched 设为 false。如果章节正文下方紧跟着【本页表格】markdown 表格段，请用该 markdown 表格作为 template 内容（而不是被换行打散的同列文字）。
6. section_type 推断规则：
